...
// Primary status for the map/progress: Issue wins, then the
...
function AppMain({ authUser }: { authUser: AuthUser }) {
  ...
  // Commissioned=1). Issue is a separate state and contributes 0.
  // "Verified" = the Commissioned share only.
  const stringProgress = useMemo(() => {
    const stages = STRING_STATUS_ORDER.filter((k) => k !== "issue" && k !== "avl");
    const weight: Record<string, number> = { issue: 0 };
      ...
      {compact && (
        <div style={{ padding: "10px 10px 12px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "grid", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            {!compact && authUser.role === "admin" && (
              <button onClick={() => setActiveTab("loginlog")} title={t("nav.loginlog", "Login Activity")} aria-label={t("nav.loginlog", "Login Activity")} style={{ ...iconBtn, width: 34, height: 34 }}>
      ...
      {userMenuOpen && (
        <>
          ...
          >
            ...
            {authUser.role === "admin" && (
              <button
                onClick={() => { setUserMenuOpen(false); setActiveTab("loginlog"); if (compact) closeMobileSidebar(); }}
                style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", padding: "8px 10px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#f8fafc", color: "#0f172a", fontSize: 13, fontWeight: 700, cursor: "pointer" }}
              >
                📊 {t("nav.loginlog", "Login Activity")}
              </button>
            )}
            <LanguageSwitcher />
            <button
      ...
      <div style={{ display: activeTab === "mapgrid" ? "block" : "none" }}>
        ...
        {electricalDetailsMode && stringTopology.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: compact ? 6 : 8, marginBottom: compact ? 6 : 10, padding: compact ? "7px 10px" : 12, border: "1px solid #e2e8f0", borderRadius: 12, background: "#f8fafc" }}>
            ...
            {/* Row 2: on phone keep the compact equal-width pills (one row, no
                horizontal scroll); on desktop show the fuller status dashboard
                cards (Total Strings + a card per status with count + share). */}
            {compact ? (
