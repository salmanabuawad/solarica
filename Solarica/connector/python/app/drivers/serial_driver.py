...
class SerialDirectDriver:
    def __init__(self) -> None:
        ...
        self.captured_raw_dir = settings.local_db_file.parent / 'raw_sui'
        self.captured_raw_dir.mkdir(parents=True, exist_ok=True)
                ...
                for raw in files:
                    ...
                    if item:
                        ...
                        with self._lock:
                            ...
                            fname = f"{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{len(self._captured):03d}.sui"
                            (self.captured_raw_dir / fname).write_bytes(raw)
    ...
    def list_files(self) -> list[dict]:
        files = []
        if self.captured_raw_dir.exists():
            for p in sorted(self.captured_raw_dir.glob("*.sui")):
                files.append(
                    {
                        "name": p.name,
                        "size": p.stat().st_size,
