...
from .database import SessionLocal, MeasurementRecord, CurvePointRecord, SyncStateRecord
...
def upsert_measurement(payload: dict) -> None:
    with SessionLocal() as session:
        ...
        if existing is None:
            existing = MeasurementRecord(id=payload['id'], measured_at=datetime.fromisoformat(payload['measuredAt']))
            session.add(existing)
        existing.external_measurement_key = payload.get('externalMeasurementKey')
        existing.measured_at = datetime.fromisoformat(payload['measuredAt'])
        ...
        session.execute(delete(CurvePointRecord).where(CurvePointRecord.measurement_id == payload['id']))
        for point in payload.get('curvePoints', []):
            session.add(CurvePointRecord(
...
def list_measurements() -> list[dict]:
    with SessionLocal() as session:
        rows = session.scalars(select(MeasurementRecord).order_by(MeasurementRecord.measured_at.desc())).all()
        return [serialize_measurement(session, row.id) for row in rows]
...
def get_measurement(measurement_id: str) -> dict | None:
    with SessionLocal() as session:
        ...
        if row is None:
            return None
        return serialize_measurement(session, measurement_id)


def serialize_measurement(session, measurement_id: str) -> dict:
    row = session.get(MeasurementRecord, measurement_id)
    points = session.scalars(select(CurvePointRecord).where(CurvePointRecord.measurement_id == measurement_id).order_by(CurvePointRecord.point_index)).all()
    return {
        'id': row.id,
        ...
        'curvePoints': [
            {'pointIndex': p.point_index, 'voltageV': p.voltage_v, 'currentA': p.current_a}
            for p in points
        ]
    }
...
def count_unsynced() -> int:
    with SessionLocal() as session:
        return int(session.scalar(select(func.count()).select_from(MeasurementRecord).where(MeasurementRecord.sync_status != 'synced')) or 0)
